function solve() {
    ...
    function onClick () {
       ...
       for(let line of input) {
          ...
          for(let worker of workersData) {
             ...
             if (output.hasOwnProperty(restaurantName)) {
                ...
                output[restaurantName][name] = Number(sallary);               
             }
          }
       }
       ...
       for(let entry of entries) {
          ...
          for(let [name, sallary] of values) {
 
             totalSallary += sallary;
          }
 
          avrSallary = totalSallary / values.length;
          
          if (avrSallary > currAvgSalary) {
 
             currAvgSalary = avrSallary;
             bestName = key;
             totalSallary = 0;
